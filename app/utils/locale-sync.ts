 ...
 */
export const syncLocaleFromPreferences = async (preferences: Preferences): Promise<void> => {
  ...
  if (i18n.language !== locale) {
    try {
      ...
      if (!i18n.hasResourceBundle(locale, 'app')) {
        const [{ default: appResources }, { default: assetResources }] = await Promise.all([
...
export const initializeLocaleFromStorage = async (): Promise<void> => {
  ...
  if (cookieLocale && cookieLocale !== i18n.language) {
    try {
      ...
      console.error(`Failed to initialize locale from cookie ${cookieLocale}:`, error);
    }
  }
};
