...
import { calculateSquareTileOffset, calculateSquareTilePoints } from 'utils/game/map-helpers';

type ContextType = {
  navigationElement: React.RefObject<HTMLElement>;
};

const BASE_TILE_SIZE: number = 75;

export const SquareMap: React.FC = () => {
  const {
    width,
    height
  } = useWindowSize();
  ...
  const [tileMagnification, setTileMagnification] = useState<1 | 2 | 3>(1);
  ...
  const tileSize = useMemo<Size>(() => {
    return {
      width: tileMagnification * BASE_TILE_SIZE,
      height: tileMagnification * BASE_TILE_SIZE
    };
  }, [tileMagnification]);
  ...
  const canvasSize = useMemo<Size>(() => {
    return {
      width: tileSize.width * (server!.configuration.mapSize),
      height: tileSize.height * (server!.configuration.mapSize)
    };
  }, [server, tileSize.height, tileSize.width]);
  ...
  const draw = (data: Tile[], context: CanvasRenderingContext2D, canvasDimensions: Size, tileDimensions: Size): void => {
    ...
    data.forEach((tile: Tile) => {
      ...
      const {
        x: offsetX,
        y: offsetY
      } = calculateSquareTileOffset(tile, canvasDimensions, tileDimensions);
      const [point1, point2, point3, point4] = calculateSquareTilePoints(offsetX, offsetY, tileDimensions);
      ...
      const textPosition: Point = {
        x: offsetX + tileSize.width - 45,
        y: offsetY + tileSize.height - 20
      };
