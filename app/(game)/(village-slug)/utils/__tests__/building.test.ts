...
describe('Building utils', () => {
  ...
  describe('calculateBuildingEffectValues', () => {
    test('CITY_WALL effect values', () => {
      ...
      expect(result.some((e) => e.effectId === 'infantryDefence' && e.currentLevelValue === 200)).toBe(true);
    });
    ...
    test('BAKERY wheat production bonus', () => {
      ...
      expect(result.some((e) => e.effectId === 'wheatProduction' && e.currentLevelValue === 1.25)).toBe(true);
    });
    ...
    test('BAKERY wheat production bonus values are increasing', () => {
      ...
      expect(result.find((e) => e.effectId === 'wheatProduction' && e.currentLevelValue === 1.15)!.areEffectValuesRising).toBe(true);
    });
  });
