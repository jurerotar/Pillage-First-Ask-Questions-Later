...
type DecodeGraphicsPropertyReturn = {
  oasisResource: Resource;
  oasisGroup: number;
  oasisGroupPositions: string;
  variant: number;
};

export const decodeGraphicsProperty = (
  encoded: number,
): DecodeGraphicsPropertyReturn => {
...
type CalculateGridLayoutReturn = {
  gridSize: number;
  totalSize: number;
  halfSize: number;
  borderWidth: number;
  totalTiles: number;
  mapBorderThreshold: number;
};

export const calculateGridLayout = (
  mapSize: number,
): CalculateGridLayoutReturn => {
...
export const tileIdToCoordinates = (
  tileId: number,
  mapSize: number,
): Coordinates => {
