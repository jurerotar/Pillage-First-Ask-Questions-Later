...
export const getResearchedUnits: ApiHandler<UnitResearch[], 'villageId'> = async (queryClient, { params }) => {
  const { villageId: villageIdParam } = params;
  const villageId = Number.parseInt(villageIdParam);

  const researchedUnits = queryClient.getQueryData<UnitResearch[]>([unitResearchCacheKey])!;
