...
export const worldItemsFactory = ({
  server,
  prng,
  occupiedOccupiableTiles,
}: WorldItemsFactoryArgs): WorldItem[] => {
  ...
  // Artifacts
  const epicHeroItemCandidates = tilesWithSize.filter(
    ({ size }) => size === '4xl',
  );
  ...
  // Rare hero items
  const rareHeroItemCandidates = tilesWithSize.filter((tile) =>
  ...
  // Uncommon hero items
  const uncommonHeroItemCandidates = tilesWithSize.filter((tile) =>
  ...
  // Common hero items
  const commonHeroItemCandidates = tilesWithSize.filter((tile) =>
