...
describe('Building assets', () => {
  test('Each building has a wheatProduction effect at index 0 of building.effects', () => {
    for (const building of buildings) {
      ...
      expect(
        firstEffect,
        `Missing effect at index 0 in ${building.id}`,
      ).toBeDefined();
      expect(
        firstEffect.effectId,
        `First effect in ${building.id} is not wheatProduction`,
      ).toBe('wheatProduction');
    }
  });
  ...
  test('The wheatProduction effect at index 0 has only 0 or negative values', () => {
    for (const building of buildings) {
      const firstEffect = building.effects[0];
      expect(
        Array.isArray(firstEffect.valuesPerLevel),
        `valuesPerLevel missing or not an array in ${building.id}`,
      ).toBe(true);

      for (const [index, value] of firstEffect.valuesPerLevel.entries()) {
        expect(
          value,
          `Value at index ${index} in ${building.id} is greater than 0`,
  ...
  test("Each building's wheatProduction effect values start with 0", () => {
    for (const building of buildings) {
      ...
      expect(
        effect.valuesPerLevel[0],
        `First value in wheatProduction of ${building.id} is not 0`,
      ).toBe(0);
    }
  });
