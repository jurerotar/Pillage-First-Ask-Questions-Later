...
import type {
  GameEvent,
  GameEventType, TroopMovementEvent,
...
export const useEventsByType = <T extends GameEventType>(eventType: T | 'troopMovement') => {
  ...
  const { data: eventsByType } = useSuspenseQuery({
    ...
    queryFn: async () => {
      const { data } = await fetcher<T extends 'troopMovement' ? TroopMovementEvent[] : GameEvent<T>[]>(
