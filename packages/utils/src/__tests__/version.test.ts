...
describe('version utils', () => {
  describe(parseAppVersion, () => {
    test('should correctly parse a standard semver-like version string', () => {
      ...
      expect(parseAppVersion('10.20.30')).toEqual([10, 20, 30]);
      expect(parseAppVersion('0.0.1')).toEqual([0, 0, 1]);
    });

    test('should handle version strings with leading zeros', () => {
      expect(parseAppVersion('01.02.03')).toEqual([1, 2, 3]);
    });
  });
  ...
  describe(parseDatabaseUserVersion, () => {
    test('should correctly parse a numeric database version', () => {
      ...
      expect(parseDatabaseUserVersion(1_002_003)).toEqual([1, 2, 3]);
      // 10 * 1_000_000 + 20 * 1_000 + 30 = 10_020_030
      expect(parseDatabaseUserVersion(10_020_030)).toEqual([10, 20, 30]);
      // 0 * 1_000_000 + 0 * 1_000 + 1 = 1
      expect(parseDatabaseUserVersion(1)).toEqual([0, 0, 1]);
    });
  });
