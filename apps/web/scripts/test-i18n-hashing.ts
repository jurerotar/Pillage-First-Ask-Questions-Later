...
console.log(`   Transformation successful: ${Object.keys(transformed).every(key => /^[a-zA-Z0-9]{6}$/.test(key)) ? '✓' : '✗'}\n`);

// Test 4: Key mapping creation
console.log('4. Testing key mapping:');
const mapping = createKeyMapping(testData);
console.log(`   Mapping created: ${Object.keys(mapping).length > 0 ? '✓' : '✗'}`);
console.log(`   Sample mapping: "${Object.keys(mapping)[0]}" -> "${mapping[Object.keys(mapping)[0]]}"\n`);
...
console.log(`   Interpolation preserved: ${transformedValue.includes('{{variable}}') ? '✓' : '✗'}\n`);
...
console.log(`   Nested structure preserved: ${typeof nestedTransformed[generateHashKey('Level1')] === 'object' ? '✓' : '✗'}`);
console.log(`   Deep nesting preserved: ${nestedTransformed[generateHashKey('Level1')][generateHashKey('Level2')][generateHashKey('Level3')] === 'Deep Value' ? '✓' : '✗'}\n`);
...
console.log(`   No collisions: ${hashes.length === uniqueHashes.size ? '✓' : '⚠'}\n`);
