...
async function migrateLocalizationFiles(options: MigrationOptions): Promise<void> {
  const { sourceDir, targetDir, locales = ['en-US'], namespaces = [] } = options;
  ...
  for (const locale of locales) {
    ...
    if (namespaceList.length === 0) {
      ...
      try {
        ...
        console.error(`Could not read extracted directory for locale ${locale}:`, error);
        continue;
      }
    }
    ...
    for (const namespace of namespaceList) {
      const sourceFile = join(sourceDir, locale, 'extracted', `${namespace}.json`);
      const targetFile = join(targetDir, locale, 'hashed', `${namespace}.json`);
      const mappingFile = join(targetDir, locale, 'hashed', `${namespace}.mapping.json`);
      ...
      try {
        ...
        if ((error as any).code === 'ENOENT') {
          console.warn(`  ⚠ Source file does not exist: ${sourceFile}, skipping...`);
        } else {
...
async function main(): Promise<void> {
  ...
  const sourceDir = args.find(arg => arg.startsWith('--source='))?.split('=')[1] || './app/localization/locales';
  const targetDir = args.find(arg => arg.startsWith('--target='))?.split('=')[1] || './app/localization/locales';
  const localesArg = args.find(arg => arg.startsWith('--locales='))?.split('=')[1];
  const namespacesArg = args.find(arg => arg.startsWith('--namespaces='))?.split('=')[1];
