...
import type { GameEvent, GameEventType, GameEventTypeToEventArgsMap } from 'app/interfaces/models/game/game-event';
import { eventsCacheKey } from 'app/(game)/(village-slug)/constants/query-keys';
import { use } from 'react';
import { ApiContext } from 'app/(game)/providers/api-provider';

type CreateEventArgs<T extends GameEventType> = Omit<GameEventTypeToEventArgsMap<T>, 'villageId' | 'type'>;
...
export const useCreateEvent = <T extends GameEventType>(eventType: T) => {
  ...
  const { mutate: createEvent } = useMutation<void, Error, SendEventArgs<T>>({
    mutationFn: async (args) => {
      const { cachesToClearImmediately, ...eventBody } = args;
