...
async function testSampleLocalization() {
  ...
  try {
    ...
    console.log(
      `   Keys match: ${Object.keys(appJson).length === Object.keys(hashedAppJson).length ? '✓' : '✗'}\n`,
    );
    ...
    const valuesPreserved = originalValues.every(
      (val, idx) => val === hashedValues[idx],
    );
    ...
    for (const key of sampleKeys) {
      const hashedKey = Object.keys(hashedAppJson).find(
        (hk) => hashedAppJson[hk] === appJson[key],
      );
      if (hashedKey) {
    ...
    const interpolationKeys = Object.keys(appJson).filter(
      (key) => key.includes('{{') && key.includes('}}'),
    );

    if (interpolationKeys.length > 0) {
      console.log(
        `   Found ${interpolationKeys.length} keys with interpolation`,
      );
    ...
    console.log(
      `   Bytes saved: ${(originalSize - hashedSize).toLocaleString()}\n`,
    );
