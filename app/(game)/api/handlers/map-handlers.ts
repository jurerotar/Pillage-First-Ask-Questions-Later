...
import { tileTypeSchema } from 'app/interfaces/models/game/tile';
...
export const getTileTroops: ApiHandler<'tileId'> = (database, { params }) => {
  const { tileId } = params;
...
export const getTileOasisBonuses: ApiHandler<'tileId'> = (
  database,
  { params },
) => {
  const { tileId } = params;
...
export const getTileWorldItem: ApiHandler<'tileId'> = (
  database,
  { params },
) => {
  const { tileId } = params;
